...
func Cetlist(context *gin.Context) {
	...
	Fdb := DB().Table("m_shopping_order").Where("accountID", user.Accountid)
	if createtime != "" {
	...
	if err != nil {
		...
		totalCount, _ = DB().Table("m_shopping_order").Where("accountID", user.Accountid).Count()
...
func Delcontent_v1(context *gin.Context) {
	...
	ids := parameter["ids"]
	res2, err := DB().Table("m_shopping_order").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
func Upstatus_v1(context *gin.Context) {
	...
	res2, err := DB().Table("m_shopping_order").WhereIn("id", ids_arr).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
