...
func Getlist(context *gin.Context) {
	...
	getfield := "id,accountID,uid,cid,title,des,keywords,thumb,flag,price,original_price,stock_num,unit,browse,evaluate,weigh,status,createtime,useway,usetimes"
	if cid == 0 {
...
func Getcontentmore(context *gin.Context) {
	...
	contentdata, _ := DB().Table("m_shopping_goods_content").Where("id", id).Fields("photo,des,content,skulist").First()
...
func Pushcontent_v1(context *gin.Context) {
	...
	//规格
	postsku := parameter["postsku"]
	delete(parameter, "postsku")
	if f_id == 0 {
		...
		if err != nil {
			...
			add_skuitem(int64(f_id), postsku) //规格
...
//批量提交-规格
func add_skuitem(fid int64, skulist interface{}) {
	save_arr := []map[string]interface{}{}
	for _, val := range skulist.([]interface{}) {
		narr := val.(map[string]interface{})
		narr["product_id"] = fid
		rule_table_key, _ := utils.JsonMarshalNoSetEscapeHTML(narr["rule_table_key"])
		narr["rule_table_key"] = rule_table_key
		save_arr = append(save_arr, narr)
	}
	DB().Table("m_shopping_goods_content_sku").Where("product_id", fid).Delete()
	if len(save_arr) > 0 {
		DB().Table("m_shopping_goods_content_sku").Data(save_arr).Insert()
	}
}
