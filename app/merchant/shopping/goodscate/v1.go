...
func Addcate_v1(context *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := DB().Table("m_shopping_goods_cate").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加失败", err)
		} else {
			if addId != 0 {
				DB().Table("m_shopping_goods_cate").
		...
		res, err := DB().Table("m_shopping_goods_cate").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func Getcate_v1(context *gin.Context) {
	...
	user := getuser.(*utils.UserClaims)
	list, _ := DB().Table("m_shopping_goods_cate").Where("accountID", user.Accountid).Order("weigh asc,id asc").Get()
	for k, val := range list {
		child_ids, _ := DB().Table("m_shopping_goods_cate").Where("pid", val["id"]).Pluck("id")
		cate_ids := child_ids.([]interface{})
		cate_ids = append(cate_ids, val["id"])
		getcount, _ := DB().Table("m_shopping_goods_content").Where("type", datatype).WhereIn("cid", cate_ids).Count()
		val["count"] = getcount
		if k == 0 {
			totalcount, _ := DB().Table("m_shopping_goods_content").Where("type", datatype).Where("accountID", user.Accountid).Count()
...
func Getparentcate_v1(context *gin.Context) {
	getuser, _ := context.Get("user") //取值 实现了跨中间件取值
	user := getuser.(*utils.UserClaims)
	list, _ := DB().Table("m_shopping_goods_cate").Where("status", 0).Where("accountID", user.Accountid).Fields("id,pid,name").Order("id asc").Get()
...
func Upcate_v1(context *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := DB().Table("m_shopping_goods_cate").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		ty := "启用"
...
func Delcate_v1(context *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := DB().Table("m_shopping_goods_cate").Where("id", parameter["id"]).Delete()
	if err != nil {
