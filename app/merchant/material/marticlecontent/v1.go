...
func Getlist(context *gin.Context) {
	...
	Fdb := DB().Table("m_marticle_content").Where("accountID", user.Accountid)
	if createtime != "" {
	...
	getfield := "id,accountID,uid,cid,title,des,link,keywords,thumb,flag,visits,likes,weigh,status,createtime"
	if cid == 0 {
		_list, _err := Fdb.Fields(getfield).Page(pageNo).Limit(pageSize).Order("id desc").Get()
		list = _list
		err = _err
	} else {
		_cate_ids, _ := DB().Table("m_marticle_cate").Where("pid", cid).Pluck("id")
	...
	if err != nil {
		...
		totalCount, _ = DB().Table("m_marticle_content").Count()
		_pageSize := int64(pageSize)
		totalPage := totalCount / _pageSize
		for _, val := range list {
			catename, _ := DB().Table("m_marticle_cate").Where("id", val["cid"]).Value("name")
...
func Getcontentmore(context *gin.Context) {
	...
	contentdata, _ := DB().Table("m_marticle_content").Where("id", id).Fields("des,content").First()
...
func Pushcontent_v1(context *gin.Context) {
	...
	if f_id == 0 {
		...
		addId, err := DB().Table("m_marticle_content").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(context, "添加失败", err)
		} else {
			if addId != 0 {
				DB().Table("m_marticle_content").
		...
		res, err := DB().Table("m_marticle_content").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func Delcontent_v1(context *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := DB().Table("m_marticle_content").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
...
func Upstatus_v1(context *gin.Context) {
	...
	res2, err := DB().Table("m_marticle_content").WhereIn("id", ids_arr).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(context, "更新失败！", err)
	} else {
		msg := "更新成功！"
